...
// then writes them into index.html via a GitHub commit.
...
const GITHUB_REPO_OWNER = process.env.GITHUB_OWNER;
const GITHUB_REPO_NAME  = process.env.GITHUB_REPO;
...
function buildDealCard(deal, index) {
  ...
  const logoImg = `<img src="https://logo.clearbit.com/${deal.domain}" alt="${deal.brand}" onerror="this.style.display='none'">`;
  ...
  const expiryHTML = deal.expiry ? `<div class="deal-exp">${deal.expiry}</div>` : "";
...
export default async function handler() {
  try {
    ...
    const { data: fileData } = await octokit.repos.getContent({
      ...
      path:  "index.html",           // ← updated from deals-site.html
    });
